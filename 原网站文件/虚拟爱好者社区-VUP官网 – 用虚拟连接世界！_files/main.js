      ...
      more[i].onclick = (e) => {
        ...
        if (e.target.parentNode.nextElementSibling.className.indexOf(" m-show-sub") != -1) {
          e.target.parentNode.nextElementSibling.className =
            e.target.parentNode.nextElementSibling.className.replace(" m-show-sub", "");
        } else {
...
var userTools = new Vue({
  ...
  async mounted() {
    ...
    if (b2token) {
      ...
      this.$http
        .post(b2_rest_url + "getUserInfo", "ref=" + ref)
        .then((res) => {
          this.role = res.data;
...
function postCatSelect() {
  ...
  if (cat.length > 0) {
    ...
    cat.forEach((_e) => {
      let _pages = document
        .querySelector("#post-item-" + _e.getAttribute("data-i"))
          ...
          if (res.status == 200) {
            ...
            if (type === "more") {
              ...
              if (res.data.data == "") {
                if (opt["post_type"] == "post-6") {
                  box.innerHTML =
                    '<tr><td colspan="10" style="border:0">' + b2_global.empty_page + "</td></tr>";
                } else {
          ...
          if (!!(window.history && history.pushState)) {
            if (page != 1) {
              ...
              if (this.navtype === "comment") {
                ...
                if (this.navtype != "authorComments" && this.title) {
                  document.title =
                    this.title + " " + b2_global.site_separator + " " + b2_global.site_name;
                }
  ...
  mounted() {
    if (this.$refs.socialBox) {
      ...
      if (code) {
        this.locked = true;
        this.$http
          .post(b2_rest_url + "socialLogin", "code=" + code + "&type=" + this.type)
          .then((res) => {
            this.locked = false;
            if (res.data === true) {
              this.back();
...
var b2GG = new Vue({
  ...
  mounted() {
    this.$http
      .post(b2_rest_url + "getLatestAnnouncement")
      .then((res) => {
        ...
        this.$http.post(b2_rest_url + "payCheck", "order_id=" + value).then((res) => {
          if (res.data.status === "success") {
            if (!userTools.userData.user_link) {
              localforage
                .getItem("b2_guest_buy_" + res.data.id + "_" + res.data.type)
                .then((_value) => {
                  let list = _value;
  ...
  methods: {
    getData() {
      this.$http.post(b2_rest_url + "getUserWidget").then((res) => {
        ...
        if (res.data.oauth) {
          if (
            !res.data.oauth.weibo.open &&
            !res.data.oauth.qq.open &&
            !res.data.oauth.weixin.open
          ) {
            this.data.oauth = [];
          }
        }
...
var b2Mission = new Vue({
  ...
  methods: {
    getData(count, paged) {
      ...
      this.$http
        .post(b2_rest_url + "getUserMission", "count=" + count + "&paged=" + this.paged)
        .then((res) => {
          this.data = res.data;
    ...
    getList() {
      ...
      this.$https
        .post(
          ...
          "paged=" + this.paged + "&count=" + this.count + "&hidden=" + this.hidden
        )
        .then((res) => {
  ...
  methods: {
    ...
    close() {
      if (
        ...
        this.$refs.asideContainer.className = this.$refs.asideContainer.className.replace(
          " aside-show",
          ""
        );
        // if(B2ClientWidth < 768){
        ...
        if (this.carts.data) {
          if (this.carts.count > Object.keys(this.carts.data).length) {
            b2mobileFooterMenu.msg =
              b2mobileFooterMenu.msg - (this.carts.count - Object.keys(this.carts.data).length);
          } else {
            b2mobileFooterMenu.msg =
              b2mobileFooterMenu.msg + (Object.keys(this.carts.data).length - this.carts.count);
          }
...
function b2HiddenFilterBox(event) {
  event.parentNode.parentNode.className = event.parentNode.parentNode.className.replace(
    "b2-show",
    ""
  );
  ...
  methods: {
    ...
    go(even, index) {
      if (typeof b2CirclePostBox != "undefined") {
        ...
        document.title =
          this.data[index].name + " " + b2_global.site_separator + " " + b2_global.site_name;
        b2AsideBar.goTop();
        return false;
      }
    },
